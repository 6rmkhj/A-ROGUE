...
enum SfxId {
    ...
    // 볼륨 마운트 연출. 위의 삽입 연출이 이미 드라이브 한 대의 소리 어휘를
    // 갖춰 두었고(활강·걸쇠·모터·헤드 이동·채터), 마운트는 같은 기계 안에서
    // 일어나는 일이라 그 어휘를 그대로 쓴다 - 빌려 쓰는 것이 아니라 같은 물건이
    // 같은 소리를 내는 것이다. 아래 넷은 삽입 연출에 없던 사건뿐이다.
    SFX_MOUNT_SPIN,    // 스핀들이 정지에서 회전수까지 올라붙는다 (1200ms)
    SFX_MOUNT_LAND,    // 액추에이터가 도는 판 위에 내려앉는다
    SFX_MOUNT_ROT,     // 헤드가 손상 섹터를 밟는다. 판독이 갈린다
    SFX_MOUNT_LAW,     // 볼륨 법칙이 판 한가운데에 각인된다
    SFX_COUNT
};
