...
data = data.replace('TSR_HIMEM = 0,   // 용량 한도 +60B', 'TSR_HIMEM = 0,   // 용량 한도 +30B')
data = replace_once(data,
'{L"HIMEM.SYS", L"용량 한도 +60B",                 20, 60, -1,',
'{L"HIMEM.SYS", L"용량 한도 +30B",                 20, 30, -1,',
"HIMEM balance")
data = replace_once(data,
...
for rel in ("translations.tsv", "build/translations.tsv"):
    ...
    t = t.replace("용량 한도 +60B\tCapacity limit +60B", "용량 한도 +30B\tCapacity limit +30B")
